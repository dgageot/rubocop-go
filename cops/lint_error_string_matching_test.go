...
`
	offenses := coptest.RunTyped(t, NewLintErrorStringMatching(), src)
...
`
		require.Len(t, coptest.RunTyped(t, NewLintErrorStringMatching(), src), 1, function)
	}
}
...
`
	require.Len(t, coptest.RunTyped(t, NewLintErrorStringMatching(), src), 1)
}
...
`
	assert.Empty(t, coptest.RunTyped(t, NewLintErrorStringMatching(), src))
}
